...
public class DiscoveryRunReaper {
    ...
    private boolean failWorkLost(UUID uuid) {
        try {
            ReapedRun reaped = transactionHandler.runInNewTransaction(() -> {
                ...
                List<MetadataAttribute> meta = run.getCheckpoint();
            ...
            // Sent even with no handle to replay: control calls are addressed by runId, with the checkpoint optional,
            // so a connector that started scanning before the handle was persisted can still be told to stop.
     ...
     * here, so restoring the pre-wipe checkpoint only feeds the cancel call, never the database. A failure is only a
    ...
    private void bestEffortConnectorCancel(ReapedRun reaped) {
        ...
        try {
            reaped.run().setCheckpoint(reaped.meta());
